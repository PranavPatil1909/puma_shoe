...
export default function WasdControls() {
  ...
  useFrame((_, delta) => {
    ...
    if (code.current.has('KeyW')) moveForward(delta * speed/6)
    if (code.current.has('KeyA')) moveRight(-delta *  speed/6 )
    if (code.current.has('KeyS')) moveForward((-delta * speed) / 6)
    if (code.current.has('KeyD')) moveRight((delta * speed) / 6)
  })
  return null
}
